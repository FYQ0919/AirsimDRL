        ...
        while True:
            try:
                ...
                while not done:
                    ...
                    observe, reward, done, info,force = env.step(transform_action(real_action))
                    image, vel = observe
                    try:
                        image = transform_input(image, args.img_height, args.img_width)
                    except:
        ...
        while True:
            try:
                ...
                while not done and timestep < time_limit:
                    ...
                    observe, reward, done, info,force= env.step(real_action)
                    image, vel = observe
                    try:
                ...
                if highscoreX < bestX:
                    ...
                    with open('save_stat/' + agent_name + '_highscore.csv', 'w', encoding='utf-8', newline='') as f:
                        ...
                        wr.writerow('%.4f' % s if type(s) is float else s for s in
                                    [highscoreX, episode, score, dt.now().strftime('%Y-%m-%d %H:%M:%S')])
